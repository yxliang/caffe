...
void FrcnnParam::set_default_param() {
	std::vector<float> v_tmp;

	FrcnnParam::scales.push_back(600.f);
	FrcnnParam::max_size = 1000.f;
	FrcnnParam::batch_size = 128.f;

	FrcnnParam::fg_fraction = 0.25f;
	FrcnnParam::fg_thresh = 0.5f;
	FrcnnParam::bg_thresh_hi =0.5f;
	FrcnnParam::bg_thresh_lo = 0.0f;
	FrcnnParam::use_flipped = true;

	FrcnnParam::bbox_reg = true;
	FrcnnParam::bbox_thresh = 0.5f;
	FrcnnParam::snapshot_infix = "";
	FrcnnParam::bbox_normalize_targets = true;
	v_tmp.push_back(1.0f);
	v_tmp.push_back(1.0f);
	v_tmp.push_back(1.0f);
	v_tmp.push_back(1.0f);
	std::copy(v_tmp.begin(), v_tmp.end(), FrcnnParam::bbox_inside_weights);
	...
	v_tmp.clear();
	v_tmp.push_back(0.1f);
	v_tmp.push_back(0.1f);
	v_tmp.push_back(0.2f);
	v_tmp.push_back(0.2f);
	std::copy(v_tmp.begin(), v_tmp.end(), FrcnnParam::bbox_normalize_stds);

	FrcnnParam::rpn_positive_overlap = 0.7f;
	FrcnnParam::rpn_negative_overlap = 0.3f;
	FrcnnParam::rpn_clobber_positives = false;
	FrcnnParam::rpn_fg_fraction = 0.5f;
	FrcnnParam::rpn_batchsize = 256;
	FrcnnParam::rpn_nms_thresh = 0.7;
	FrcnnParam::rpn_pre_nms_top_n = 12000;
	FrcnnParam::rpn_post_nms_top_n = 2000;
	FrcnnParam::rpn_min_size = 16;
	...
	v_tmp.push_back(1.0f);
	std::copy(v_tmp.begin(), v_tmp.end(), FrcnnParam::rpn_bbox_inside_weights);
	FrcnnParam::rpn_positive_weight = -1.0f;
	FrcnnParam::rpn_allowed_border = 0.0f;

	// ======================================== Test
	FrcnnParam::test_scales.push_back(600);
	FrcnnParam::test_max_size = 1000.f;
	FrcnnParam::test_nms = 0.3f;

	FrcnnParam::test_bbox_reg = true;
	FrcnnParam::test_rpn_nms_thresh = 0.3f;
	FrcnnParam::test_rpn_pre_nms_top_n = 6000;
	FrcnnParam::test_rpn_post_nms_top_n = 300;
	FrcnnParam::test_rpn_min_size = 16.f;

	// ========================================
	v_tmp.clear();
	v_tmp.push_back(102.9801f);
	v_tmp.push_back(115.9465f);
	v_tmp.push_back(122.7717f);
	std::copy(v_tmp.begin(), v_tmp.end(), FrcnnParam::pixel_means);
	FrcnnParam::rng_seed = 3;
	FrcnnParam::eps = 0.00000000000001f;
	FrcnnParam::inf = 100000000.f;

	// ========================================
	FrcnnParam::feat_stride = 16;
	float _anchors[] = { -84, -40, 99, 55,
		-176,  -88,  191,   103,
		-360,  -184,   375,   100,
		-56,   -56,    71,    72,
		-120,  -120,   135,   135,
